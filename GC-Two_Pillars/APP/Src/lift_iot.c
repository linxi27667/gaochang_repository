...
/* 注意：本函数假设调用方已经持有 IoT 锁（g_lift_iot_status / g_iot_event_pending 串行访问） */
                                                       ...
                                                       int16_t csq)
{
    ...
    /* 读取两柱 3 路输出（电磁铁复用 IO_OUT_MAIN_AIR_VALVE 槽位） */
    uint8_t out_motor        = App_IO_Read_Output(IO_OUT_MOTOR);
    ...
    len = snprintf(buf, size,
                   ...
                   "\"safety\":{"
                   "\"alarm\":\"%s\",\"alarm_code\":%u,"
                   "\"upper\":%u,\"lower\":0,\"estop\":%u,\"photoelectric\":0"
                   "},"
                   "\"stats\":{"
                                             ...
                                             uint32_t seq)
{
    ...
    /* 大剪无编码器，返回 stub JSON */
    len = snprintf(buf, size,
                   "{\"type\":\"height\",\"device\":\"%s\",\"uid\":\"%s\",\"chip_uid\":\"%s\","
                   "\"seq\":%lu,\"tick\":%lu,"
