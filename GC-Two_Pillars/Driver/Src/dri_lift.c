...
/* 诊断标志：J-Link 读取确认任务是否真的被创建并运行 */
volatile uint8_t g_lift_task_created = 0;    /* Lift_Task_Create 调用后置 1 */
volatile uint8_t g_lift_task_started = 0;    /* Lift_Task 入口执行后置 1 */
volatile uint32_t g_lift_task_loop_cnt = 0;  /* 主循环计数（每 10ms +1） */
...
void Lift_Task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        /* The counter worker owns Flash; this is state observation only. */
        App_RiseCounter_Poll(state_snap.state, HAL_GetTick());
        ...
        {
            heartbeat_cnt = 0U;
            LIFT_HEARTBEAT_LOG_I("[LIFT] heartbeat loop=%lu state=%d dtu=%s led=%d",
