...
static uint8_t role_lower_limit_active(lift_role_t role)
{
    return (role == LIFT_ROLE_MAIN) ? App_IO_Read(IO_IN_LOWER_LIMIT) : 0U;
}
...
static void ls_poll_down(lift_role_t role)
{
     ...
     * 电机断开，工作阀+气阀+下降阀立即打开
     */
    /* Upper/lock are sampled for diagnostics/output hold only; do not skip pre-lift timing. */
    uint8_t upper_limit = App_IO_Read(role_upper_limit(role));
     ...
     * 车辆已落地，立即全停回 IDLE
     */
    uint8_t lower_limit = role_lower_limit_active(role);
    if (lower_limit) {
        uint16_t dur = (uint16_t)(now - s_action_start_tick);
        ls_stop_all_to_idle();
        uint8_t detail = (uint8_t)role;
    ...
    if ((s_ls_state == LS_DOWN_DROPPING) && (upper_limit || lock_pressed)) {
        /* 强制下降：电机断开，所有阀打开 */
        App_IO_Write(IO_OUT_MOTOR, 0);
        App_IO_Write(role_work_valve(role), 1);
        App_IO_Write(role_air_valve(role), 1);
