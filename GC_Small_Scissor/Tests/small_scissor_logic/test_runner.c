...
static void test_refill(void)
{
    ...
    expect_output("refill start", 1, 0, 1);
...
static void test_refill_ignores_upper_limit(void)
{
    ...
    expect_output("refill despite upper", 1, 0, 1);
...
static void test_photo_alarm_and_clear(void)
{
    ...
    expect_log("Photoelectric blocked");
...
static void test_up_down_interlock(void)
{
    ...
    expect_log("up_down_same_time");
}

static void test_maintenance_counts_only_continuous_rise(void)
{
    lift_iot_maintenance_t maintenance;

    start_case("maintenance_counts_only_continuous_rise");
    reset_fixture();
    ...
    TEST_EXPECT(&g_stats, maintenance.total_lift_count == 0U,
                "separate rises must not combine into a maintenance count");
    ...
    run_for_ms(40U);
    LiftIot_GetMaintenance(&maintenance);
    TEST_EXPECT(&g_stats, maintenance.total_lift_count == 1U,
                "one continuous rise of three seconds should count once");
}

static void test_maintenance_threshold_and_power_recovery(void)
{
    lift_iot_maintenance_t maintenance;
    w25q_maintenance_ledger_t persisted;
    uint32_t index;

    start_case("maintenance_threshold_and_power_recovery");
    reset_fixture();

    for (index = 0U; index < (W25Q_MAINTENANCE_THRESHOLD - 1U); index++) {
        TEST_EXPECT(&g_stats, App_W25Qxx_Maintenance_Increment(&persisted) == W25Q_OK,
                    "pre-threshold ledger increment should persist");
    }
    LiftIot_Init();
    LiftIot_GetMaintenance(&maintenance);
    TEST_EXPECT(&g_stats, maintenance.maintenance_due == 0U,
                "4999 counts should not trigger maintenance");
    ...
    run_for_ms(40U);
    LiftIot_GetMaintenance(&maintenance);
    TEST_EXPECT(&g_stats, maintenance.total_lift_count == W25Q_MAINTENANCE_THRESHOLD,
                "threshold rise should persist the total count");
    TEST_EXPECT(&g_stats, maintenance.maintenance_due == 1U,
                "5000 counts should trigger a reminder without locking motion");

    LiftIot_Init();
    LiftIot_GetMaintenance(&maintenance);
    TEST_EXPECT(&g_stats, maintenance.total_lift_count == W25Q_MAINTENANCE_THRESHOLD,
                "restart should recover the latest committed ledger record");
}

static void test_maintenance_corrupt_record_and_commands(void)
{
    lift_iot_maintenance_t maintenance;
    w25q_maintenance_ledger_t persisted;

    start_case("maintenance_corrupt_record_and_commands");
    reset_fixture();

    TEST_EXPECT(&g_stats, App_W25Qxx_Maintenance_Increment(&persisted) == W25Q_OK,
                "first ledger record should persist");
    TEST_EXPECT(&g_stats, App_W25Qxx_Maintenance_Increment(&persisted) == W25Q_OK,
                "second ledger record should persist");
    fake_w25qxx_corrupt_latest();
    LiftIot_Init();
    LiftIot_GetMaintenance(&maintenance);
    TEST_EXPECT(&g_stats, maintenance.total_lift_count == 1U,
                "corrupt newest record should fall back to the prior valid record");

    TEST_EXPECT(&g_stats, LiftIot_MaintenanceDone("test", "done-1") == LIFT_IOT_OK,
                "maintenance confirmation should persist");
    TEST_EXPECT(&g_stats, LiftIot_MaintenanceDone("test", "done-1") == LIFT_IOT_OK,
                "duplicate maintenance confirmation should be idempotent");
    LiftIot_GetMaintenance(&maintenance);
    TEST_EXPECT(&g_stats, maintenance.maintenance_count == 1U,
                "duplicate confirmation must not increment maintenance count twice");
    TEST_EXPECT(&g_stats, maintenance.last_maintenance_total == 1U,
                "maintenance confirmation should retain the total count");

    TEST_EXPECT(&g_stats, LiftIot_ResetUsage("test", "reset-1") == LIFT_IOT_OK,
                "usage reset should persist");
    TEST_EXPECT(&g_stats, LiftIot_ResetUsage("test", "reset-1") == LIFT_IOT_OK,
                "duplicate usage reset should be idempotent");
    LiftIot_GetMaintenance(&maintenance);
    TEST_EXPECT(&g_stats, maintenance.total_lift_count == 0U &&
                maintenance.maintenance_lift_count == 0U &&
                maintenance.maintenance_count == 0U &&
                maintenance.maintenance_due == 0U &&
                maintenance.usage_epoch == 1U,
                "factory reset should clear usage and advance the epoch exactly once");
}

static void test_maintenance_telemetry_contract(void)
{
    char json[1536];

    start_case("maintenance_telemetry_contract");
    reset_fixture();
    set_input(IO_IN_LOWER_LIMIT, 1);
    run_for_ms(30U);

    TEST_EXPECT(&g_stats,
                LiftIot_BuildTelemetryJson(json, sizeof(json), "telemetry", 1U, "ready", 20) == LIFT_IOT_OK,
                "telemetry must fit the DTU JSON buffer");
    TEST_EXPECT(&g_stats,
                (strlen(json) + 1U) < 1024U,
                "telemetry plus newline must stay below the DTU packet limit");
    TEST_EXPECT(&g_stats,
                strstr(json, "\"lower_limit\":1") != NULL,
                "telemetry must expose an active lower limit");
    TEST_EXPECT(&g_stats,
                strstr(json, "\"lower\":1") != NULL,
                "telemetry safety state must expose an active lower limit");
    TEST_EXPECT(&g_stats,
                strstr(json, "\"maintenance\":{\"total_lift_count\":0,\"maintenance_lift_count\":0,\"maintenance_threshold\":5000") != NULL,
                "telemetry must expose the normalized maintenance object");
    TEST_EXPECT(&g_stats,
                strstr(json, "\"maintenance_due\":0") != NULL,
                "telemetry must retain the top-level maintenance_due field");
}
...
int main(void)
{
    ...
    test_maintenance_counts_only_continuous_rise();
    test_maintenance_threshold_and_power_recovery();
    test_maintenance_corrupt_record_and_commands();
    test_maintenance_telemetry_contract();
