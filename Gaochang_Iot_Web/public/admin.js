...
function renderRegistry(list) {
    ...
    tbody.innerHTML = list.map(r => {
        ...
        const displayName = r.display_name || r.model;
        const accessories = [
            r.has_encoder ? '高度' : '',
            r.has_buzzer ? '蜂鸣' : '',
...
function editRegistry(id, data) {
    ...
    document.getElementById('registry-model').value = data.model || data.display_name || '';
...
async function submitRegistry() {
    ...
    const payload = {
        ...
        uid: document.getElementById('registry-uid').value.trim(),
        product_type: document.getElementById('registry-product-type').value,
        display_name: document.getElementById('registry-model').value.trim(),
...
function renderBindings(list) {
    ...
    tbody.innerHTML = list.map(r => {
        const ownerName = r.owner_username
            ? escapeHtml(r.owner_real_name ? `${r.owner_real_name} (${r.owner_username})` : r.owner_username)
            : '-';
        ...
        return `
            <tr>
                ...
                <td class="nowrap">
                    <span class="action-link" onclick="showProductTypeModal('${escapeAttr(r.device_id)}', '${escapeAttr(r.name)}', '${escapeAttr(r.product_type || 'double_post')}')">切换型号</span>
                    ${bound
...
// 切换设备产品型号(下发命令到设备)
function showProductTypeModal(deviceId, deviceName, currentType) {
    const options = Object.entries(PRODUCT_TYPE_NAMES)
        .map(([k, v]) => `<option value="${k}" ${k === currentType ? 'selected' : ''}>${v}</option>`)
        .join('');
    document.getElementById('confirm-title').textContent = '切换产品型号';
    document.getElementById('confirm-message').innerHTML = `
        <p>设备：<b>${escapeHtml(deviceName)}</b> <code style="font-size:11px;">${escapeHtml(deviceId)}</code></p>
        <div class="form-group" style="margin-top:12px;">
            <label>选择新产品型号</label>
            <select id="confirm-product-type" class="select" style="width:100%;">${options}</select>
        </div>
        <p class="text-muted" style="font-size:12px;margin-top:6px;">切换后会下发命令到设备,设备下次重启后生效新控制逻辑。</p>
    `;
    const btn = document.getElementById('confirm-ok-btn');
    const newBtn = btn.cloneNode(true);
    btn.parentNode.replaceChild(newBtn, btn);
    newBtn.textContent = '确认切换';
    newBtn.addEventListener('click', async () => {
        const newType = document.getElementById('confirm-product-type').value;
        closeModal('confirm-modal');
        try {
            await api('/api/commands/set_product_type/' + encodeURIComponent(deviceId), {
                method: 'POST',
                body: JSON.stringify({ product_type: newType })
            });
            toast('切换型号命令已下发,等待设备确认', 'success');
            loadBindings();
        } catch (err) {
...
async function submitResetPassword() {
    ...
    try {
        const result = await api('/api/admin/users/' + id + '/reset-password', {
            method: 'PUT',
            body: JSON.stringify(body)
        });
        let msg = '密码已重置';
        if (result.default_password) {
            msg = '密码已重置为: ' + result.default_password;
        }
...
function renderDeviceOpStats(data) {
    ...
    grid.innerHTML = (typeCards || '<div class="loading">暂无数据</div>') + (resultCards || '');
}
