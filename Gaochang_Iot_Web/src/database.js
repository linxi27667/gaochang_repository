...
const PRODUCT_CONFIGS = [
  {
    ...
    has_rotary: 0,
    has_limit_down: 0,
    firmware_folder: FIRMWARE_DISPLAY_NAMES.double_post
  },
  {
  ...
  },
  {
    ...
    has_rotary: 0,
    has_limit_down: 1,
    firmware_folder: FIRMWARE_DISPLAY_NAMES.thin_scissor
  },
  {
    product_type: 'large_scissor',
    display_name: '大剪举升机',
    inputs_json: '["btn_up","btn_down","btn_lock","estop","upper_limit","sub_upper_limit","lower_limit","refill","photoelectric","rotary"]',
    outputs_json: '["motor","main_air","sub_air","drop_valve","main_work","sub_work"]',
    default_motor_hold_ms: 3000,
    default_motor_to_valve_delay_ms: 200,
    has_encoder: 0,
    has_refill: 1,
    ...
    has_rotary: 1,
    has_limit_down: 1,
    firmware_folder: FIRMWARE_DISPLAY_NAMES.large_scissor
  }
];
...
function init() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS device_status (
      ...
      total_lift_count INTEGER DEFAULT 0,
      maintenance_lift_count INTEGER DEFAULT 0,
      maintenance_threshold INTEGER DEFAULT 5000,
      maintenance_count INTEGER DEFAULT 0,
      last_maintenance_total INTEGER DEFAULT 0,
      maintenance_due INTEGER DEFAULT 0,
      usage_epoch INTEGER DEFAULT 0,
      maintenance_revision INTEGER DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS device_operation_logs (
      ...
      role TEXT DEFAULT '',
      device_tick_ms INTEGER DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS product_configs (
      ...
      firmware_folder TEXT DEFAULT ''
    );

    -- 多对多设备绑定表(替代 devices.owner_id 单一归属)
    -- 一台设备可被多个账号绑定,一个账号可绑定多台设备
    CREATE TABLE IF NOT EXISTS device_bindings (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      device_id TEXT NOT NULL,
      user_id INTEGER NOT NULL,
      status TEXT NOT NULL DEFAULT 'active',     -- active / revoked / pending
      bind_type TEXT NOT NULL DEFAULT 'normal',  -- normal / special_approved
    ...
    CREATE INDEX IF NOT EXISTS idx_bindings_status ON device_bindings(status);

    -- 绑定审批申请表(超过 3 人绑定后走审批流程)
    CREATE TABLE IF NOT EXISTS binding_requests (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      device_id TEXT NOT NULL,
      user_id INTEGER NOT NULL,
      serial TEXT DEFAULT '',                    -- 申请时填写的 GC 序列号
    ...
    CREATE INDEX IF NOT EXISTS idx_requests_device ON binding_requests(device_id);
    CREATE INDEX IF NOT EXISTS idx_requests_user ON binding_requests(user_id);
    CREATE INDEX IF NOT EXISTS idx_requests_status ON binding_requests(status);

    -- command_queue 索引:msg_id(状态机核心查询)、status(超时检查)、device_id(命令历史)
    CREATE INDEX IF NOT EXISTS idx_cmd_queue_msg_id ON command_queue(msg_id);
    CREATE INDEX IF NOT EXISTS idx_cmd_queue_status ON command_queue(status);
    CREATE INDEX IF NOT EXISTS idx_cmd_queue_device ON command_queue(device_id);

    -- operation_logs 索引:user_id(用户审计)、created_at(时间范围)、device_id(按设备查询)
    ...
    CREATE INDEX IF NOT EXISTS idx_op_logs_device ON operation_logs(device_id);

    -- alarms 索引:device_id(按设备查询报警)
    CREATE INDEX IF NOT EXISTS idx_alarms_device ON alarms(device_id);

    -- device_operation_logs:device_serial(常用过滤字段,与 device_uid 互补)
    CREATE INDEX IF NOT EXISTS idx_devop_serial ON device_operation_logs(device_serial);
  `);
  ...
  // JWT 撤销机制:用户禁用/改角色/重置密码/删号后 auth_version+1,旧 JWT 立即失效
  ensureColumn(db, 'users', 'auth_version', 'INTEGER DEFAULT 0');

  // device_registry:出厂绑定码哈希(SHA-256+salt)与隔离状态
  ensureColumn(db, 'device_registry', 'bind_code_hash', "TEXT DEFAULT ''");
  ensureColumn(db, 'device_registry', 'isolation_status', "TEXT DEFAULT 'ok'");
  ensureColumn(db, 'device_registry', 'isolation_reason', "TEXT DEFAULT ''");
  ensureColumn(db, 'device_registry', 'last_seen_at', "TEXT DEFAULT ''");
  ensureColumn(db, 'device_registry', 'firmware_version', "TEXT DEFAULT ''");
  ensureColumn(db, 'device_registry', 'boot_id', "TEXT DEFAULT ''");

  // command_queue:命令状态机扩展字段(pending → sent → succeeded/rejected/timeout/failed)
  ensureColumn(db, 'command_queue', 'chip_uid', "TEXT DEFAULT ''");
  ensureColumn(db, 'command_queue', 'operator_id', 'INTEGER');
  ensureColumn(db, 'command_queue', 'operator_name', "TEXT DEFAULT ''");
  ensureColumn(db, 'command_queue', 'args_json', "TEXT DEFAULT '{}'");
  ensureColumn(db, 'command_queue', 'attempts', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'command_queue', 'max_attempts', 'INTEGER DEFAULT 2');
  ...
  ensureColumn(db, 'device_status', 'total_lift_count', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'device_status', 'maintenance_lift_count', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'device_status', 'maintenance_threshold', 'INTEGER DEFAULT 5000');
  ensureColumn(db, 'device_status', 'maintenance_count', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'device_status', 'last_maintenance_total', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'device_status', 'maintenance_due', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'device_status', 'usage_epoch', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'device_status', 'maintenance_revision', 'INTEGER DEFAULT 0');

  ensureColumn(db, 'maintenance_records', 'command_msg_id', "TEXT DEFAULT ''");
  ensureColumn(db, 'maintenance_records', 'total_lift_count', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'maintenance_records', 'maintenance_lift_count', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'maintenance_records', 'maintenance_count', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'device_operation_logs', 'role', "TEXT DEFAULT ''");
  ensureColumn(db, 'device_operation_logs', 'device_tick_ms', 'INTEGER DEFAULT 0');
  ensureColumn(db, 'device_operation_logs', 'message_key', "TEXT DEFAULT ''");
  db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_devop_message_key ON device_operation_logs(message_key) WHERE message_key <> ''");
  db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_maintenance_records_command_msg_id ON maintenance_records(command_msg_id) WHERE command_msg_id <> ''");
  ...
  ensureColumn(db, 'product_configs', 'firmware_folder', "TEXT DEFAULT ''");

  // 角色统一化迁移:把旧的多角色(admin/superadmin/operator/viewer)合并为两层 admin/user
  migrateRolesToTwoLayer(db);

  // 把旧的 devices.owner_id 单一归属迁移为 device_bindings 多对多绑定
  migrateOwnerToBindings(db);
  migrateUnboundDeviceStatuses(db);
  migrateRegistryDisplayNames(db);
  ...
  const upsertProduct = db.prepare(`
    INSERT INTO product_configs (product_type, display_name, inputs_json, outputs_json,
      ...
      has_encoder, has_refill, has_photoelectric, has_rotary, has_limit_down, firmware_folder)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(product_type) DO UPDATE SET
      ...
      firmware_folder=excluded.firmware_folder
  `);
...
// 把旧的 devices.owner_id 单一归属迁移为 device_bindings 多对多绑定
// 幂等:已存在的绑定记录不会重复插入(UNIQUE 约束保护)
function migrateOwnerToBindings(database) {
  // 查找所有 owner_id 非空且尚未在 device_bindings 中存在记录的设备
  const orphans = database.prepare(`
    SELECT device_id, owner_id, bound_at
    FROM devices
    WHERE owner_id IS NOT NULL
      AND owner_id != 0
      AND NOT EXISTS (
        SELECT 1 FROM device_bindings
        WHERE device_bindings.device_id = devices.device_id
          AND device_bindings.user_id = devices.owner_id
          AND device_bindings.status = 'active'
      )
  `).all();
  ...
  const insertBinding = database.prepare(`
    INSERT OR IGNORE INTO device_bindings (device_id, user_id, status, bind_type, bound_at)
  ...
  const tx = database.transaction((rows) => {
    for (const row of rows) {
      insertBinding.run(row.device_id, row.owner_id, row.bound_at || now);
      migrated++;
    }
  });
  tx(orphans);

  console.log(`[db] migrateOwnerToBindings: migrated ${migrated} rows from owner_id to device_bindings`);
}

// 旧版本以 device_id 保存未绑定上报。同一 MCU 在固件升级或协议切换后可能
// 使用不同 device_id，导致注册表按 UID 联查时展开成多行。UID 才是设备身份。
function migrateUnboundDeviceStatuses(database) {
  const dedupe = database.transaction(() => {
    database.prepare(`
      DELETE FROM unbound_device_status AS older
      WHERE older.uid <> ''
        AND EXISTS (
          SELECT 1 FROM unbound_device_status AS newer
          WHERE newer.uid = older.uid
            AND (
              COALESCE(newer.ts_ms, 0) > COALESCE(older.ts_ms, 0)
              OR (
                COALESCE(newer.ts_ms, 0) = COALESCE(older.ts_ms, 0)
    ...
    `).run();
    database.exec(`
      CREATE UNIQUE INDEX IF NOT EXISTS idx_unbound_status_uid_unique
      ON unbound_device_status(uid) WHERE uid <> ''
    `);
  });
  dedupe();
}

function migrateRegistryDisplayNames(database) {
  const update = database.prepare('UPDATE device_registry SET display_name = ? WHERE product_type = ?');
  const tx = database.transaction(() => {
    for (const [productType, displayName] of Object.entries(FIRMWARE_DISPLAY_NAMES)) {
      update.run(displayName, productType);
    }
  });
  tx();
}
...
function ensureDefaultAdmin(database) {
  ...
  // 默认网站管理员账号
  // 注意:默认密码 'admin123' 仅供首次部署登录,创建后必须立即修改
  // 通过 must_change_password = 1 强制管理员首次登录后改密
  const admin = database.prepare('SELECT id FROM users WHERE username = ?').get('admin');
  if (!admin) {
    const hash = bcrypt.hashSync('admin123', 10);
    database.prepare(
      'INSERT OR IGNORE INTO users (username, password_hash, role, real_name, created_at, must_change_password) VALUES (?, ?, ?, ?, ?, 1)'
...
module.exports = { init, getDb, PRODUCT_CONFIGS, FIRMWARE_DISPLAY_NAMES };
