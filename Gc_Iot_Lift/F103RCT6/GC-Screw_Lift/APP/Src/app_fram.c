...
static HAL_StatusTypeDef App_Fram_Read_Internal(uint16_t address,
                                                uint8_t *data,
                                                uint16_t length);
static HAL_StatusTypeDef App_Fram_Write_Internal(uint16_t address,
                                                 const uint8_t *data,
                                                 uint16_t length);
...
{
    return (App_Fram_Read_Internal(address, data, length) == HAL_OK) ? 0 : -1;
}

int App_Fram_Write(uint16_t address, const uint8_t *data, uint16_t length)
{
    return (App_Fram_Write_Internal(address, data, length) == HAL_OK) ? 0 : -1;
}
...
app_fram_test_result_t App_Fram_Self_Test(void)
{
    ...
    if (App_Fram_Is_Ready() != 0U)
    {
        return APP_FRAM_TEST_DEVICE_NOT_READY;
    }

    if (App_Fram_Read_Internal(APP_FRAM_TEST_ADDRESS,
                               backup,
    ...
    if (App_Fram_Write_Internal(APP_FRAM_TEST_ADDRESS,
                                test_pattern,
                                APP_FRAM_TEST_LENGTH) != HAL_OK)
    {
        result = APP_FRAM_TEST_WRITE_FAILED;
    }
    else if (App_Fram_Read_Internal(APP_FRAM_TEST_ADDRESS,
                                    verify,
                                    APP_FRAM_TEST_LENGTH) != HAL_OK)
    {
