...
static volatile uint8_t s_exti_collision_pending = 0;
static volatile direction_t s_exti_blocked_direction = DIR_STOP;
...
void Safety_Init(void)
{
    ...
    s_exti_collision_pending               = 0;
    s_exti_blocked_direction               = DIR_STOP;
}
...
void Safety_Check_Collision(void)
{
    ...
    /* RCT6 光耦输入：外部触发后 MCU 脚为低电平 */
    g_safety.left_up_collision    = (HAL_GPIO_ReadPin(Left_Up_Safety_GPIO_Port, Left_Up_Safety_Pin) == GPIO_PIN_RESET) ? 1U : 0U;
    g_safety.right_up_collision   = (HAL_GPIO_ReadPin(Right_Up_Safety_GPIO_Port, Right_Up_Safety_Pin) == GPIO_PIN_RESET) ? 1U : 0U;
    g_safety.left_down_collision  = (HAL_GPIO_ReadPin(Left_Down_Safety_GPIO_Port, Left_Down_Safety_Pin) == GPIO_PIN_RESET) ? 1U : 0U;
    g_safety.right_down_collision = (HAL_GPIO_ReadPin(Right_Down_Safety_GPIO_Port, Right_Down_Safety_Pin) == GPIO_PIN_RESET) ? 1U : 0U;
    ...
    if (up_raw && !last_up_raw) {
        #if SAFETY_DEBUG == 1
        elog_w("SAFETY", "[SAFETY] Upper collision ACTIVE left=%d right=%d",
               g_safety.left_up_collision, g_safety.right_up_collision);
        #endif
    }
    if (down_raw && !last_down_raw) {
        ...
        #if SAFETY_DEBUG == 1
        elog_w("SAFETY", "[SAFETY] Lower collision ACTIVE left=%d right=%d",
               g_safety.left_down_collision, g_safety.right_down_collision);
        #endif
    }
    ...
    if (s_exti_collision_pending) {
        direction_t blocked_direction = s_exti_blocked_direction;
        s_exti_collision_pending = 0;
        Safety_Report_Blocked_Move(blocked_direction);
    }
    ...
    /* 既定策略：碰撞杆离开后报警自动解除，允许在其它安全条件满足时再启动。
       不是遗漏；不要求人工/远程确认复位。 */
    if (g_safety.alarm == ALARM_COLLISION && !up_raw && !down_raw) {
        ...
        App_Buzzer_Off();
        #if SAFETY_DEBUG == 1
        elog_a("SAFETY", "[SAFETY] Collision alarm cleared by pins inactive left=%ldmm right=%ldmm",
               HEIGHT_MM(g_column[0].pulse_count), HEIGHT_MM(g_column[1].pulse_count));
        #endif
...
void Safety_EXTI_Handler(uint16_t gpio_pin)
{
    ...
    uint8_t blocks_motion =
        ((gpio_pin == Left_Up_Safety_Pin || gpio_pin == Right_Up_Safety_Pin) && direction == DIR_UP) ||
        ((gpio_pin == Left_Down_Safety_Pin || gpio_pin == Right_Down_Safety_Pin) && direction == DIR_DOWN);

    if (!blocks_motion) return;

    /* ISR path: no delay, logging, buzzer, or FreeRTOS API. */
    Motor_Stop_All_Immediate();
    g_command.direction = DIR_STOP;
    g_safety.alarm = ALARM_COLLISION;
    s_exti_blocked_direction = direction;
    s_exti_collision_pending = 1;
}
