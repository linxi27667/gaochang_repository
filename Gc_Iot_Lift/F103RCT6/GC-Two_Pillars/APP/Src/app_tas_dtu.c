...
static tas_dtu_result_t App_TasDtu_WaitMqttConnected(uint32_t timeout_ms)
{
    ...
    do
    {
        ...
        {
            DTU_CONNECT_LOG_W("DTU", "[DTU] transient ERROR while waiting MQTT, continue");
        }
                                                       ...
                                                       const char *event)
{
    ...
    len = snprintf(json, sizeof(json),
                   ...
                   "\"last_maintenance_total\":%lu,\"maintenance_due\":%u,\"usage_epoch\":%lu,\"maintenance_revision\":%lu},"
                   "\"dtu\":{\"state\":\"%s\",\"csq\":%d}}",
                   App_TasDtu_ChipUidString(),
                   App_TasDtu_ChipUidString(),
                   "double_post",
                   ...
                   (unsigned long)maintenance.maintenance_revision,
                   App_TasDtu_StateName(g_tas_dtu_status.state),
...
static uint8_t App_TasDtu_IsAcceptedCommandJson(const char *json)
{
    ...
    return App_TasDtu_UidMatchesLocal(target_uid);
}
...
static void App_TasDtu_HandleJsonCommand(const char *json)
{
    ...
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] CMD rejected: invalid type or target_uid");
        return;
    }
    ...
    if (msg_id[0] == '\0')
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] CMD rejected: missing msg_id cmd=%s",
                           (cmd_name[0] != '\0') ? cmd_name : "unknown");
    ...
    if (App_TasDtu_JsonHasCommand(json, "maintenance_done"))
    {
        ...
        (void)App_TasDtu_ReportCommandResult("maintenance_done", msg_id,
                                             (lift_result == LIFT_IOT_OK) ? "maintenance_done" : "maintenance_denied",
                                             (lift_result == LIFT_IOT_OK) ? "maintenance_done_ok" : "maintenance_done_denied");
        return;
    }
...
tas_dtu_result_t App_TasDtu_ReportRiseCount(const app_rise_counter_upload_t *upload)
{
    char json[TAS_DTU_JSON_SIZE];
    ...
    if (LiftIot_BuildRiseCountJson(json,
                                   sizeof(json),
                                   upload->seq,
                                   upload->delta,
                                   upload->total_count,
                                   upload->total_rise_ms,
                                   upload->remainder_ms,
                                   upload->usage_epoch) != LIFT_IOT_OK)
    {
        return TAS_DTU_RESULT_BUFFER_SMALL;
    }
...
tas_dtu_result_t App_TasDtu_SendJson(const char *json)
{
    ...
    if ((strlen(json) + 1U) > TAS_DTU_PACKET_SIZE)
    {
        DTU_TRANSFER_LOG_E("DTU", "[DTU] JSON exceeds packet limit bytes=%lu limit=%u",
                           (unsigned long)(strlen(json) + 1U),
