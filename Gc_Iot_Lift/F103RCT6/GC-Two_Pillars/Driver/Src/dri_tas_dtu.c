...
static void TasDtu_Task_ReportPendingRiseCount(void)
{
    app_rise_counter_upload_t upload;
    ...
    if (App_RiseCounter_GetPendingUpload(&upload) == 0U)
    {
        return;
    }

    result = App_TasDtu_ReportRiseCount(&upload);
    if (result != TAS_DTU_RESULT_OK)
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] rise count upload failed: seq=%lu result=%d",
                           (unsigned long)upload.seq,
                           (int)result);
        return;
    }

    if (App_RiseCounter_MarkUploadSent(upload.seq, upload.delta) == 0U)
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] rise count confirm failed: seq=%lu",
                           (unsigned long)upload.seq);
    }
}
...
void TasDtu_Task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        TasDtu_Task_ReportPendingRiseCount();
