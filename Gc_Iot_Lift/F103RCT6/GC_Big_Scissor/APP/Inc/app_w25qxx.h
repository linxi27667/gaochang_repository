...
/* ============ FRAM 8KB 地址规划（FM24CL64B，双槽间距=256，勿越界 0x1FF0） ============
 * 0x0000 Storage A / 0x0100 Storage B
 * 0x0200 Config A  / 0x0300 Config B
 * 0x0400 Stats A   / 0x0500 Stats B
 * 0x0600 OpLog (256B)
 * 0x0700 RiseQueue 12×256 → 至 0x12FF
 * 0x1300 Maintenance 10×256 → 至 0x1CFF
 * 0x1FF0 self-test
 */
...
#define W25Q_RISE_QUEUE_ADDR          0x0700U
#define W25Q_RISE_QUEUE_SECTOR_SIZE   0x0100U
#define W25Q_RISE_QUEUE_SECTOR_COUNT  12U

#define W25Q_MAINTENANCE_ADDR          0x1300U
#define W25Q_MAINTENANCE_SECTOR_SIZE   0x0100U
#define W25Q_MAINTENANCE_SECTOR_COUNT  10U
...
typedef struct {
    ...
    uint16_t motor_hold_ms;                /* 主机下降气阀打开后电机保持，默认 2500 */
    uint16_t sub_motor_hold_ms;            /* 子机下降气阀打开后电机保持，默认 1500 */
...
/* A qualified three-second rise event waiting for DTU delivery. */
typedef struct {
    uint32_t sequence;
    uint32_t up_total;
    uint32_t up_main;
...
/* Rise-count queue ownership stays in the DTU task; control code only enqueues. */
...
uint8_t App_W25Qxx_RiseQueue_ResetUsage(uint32_t usage_epoch);

/* Clears the unified in-RAM statistics only after reset_usage commits. */
