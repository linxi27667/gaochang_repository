...
typedef struct {
    GPIO_TypeDef *port;
    uint16_t      pin;
    uint8_t       active_level;   /* 板上光耦统一低有效 → active_level=0 */
...
typedef struct {
    GPIO_TypeDef *port;
    uint16_t      pin;
    uint8_t       active_level;   /* 继电器高有效 */
} io_out_pin_t;

/* GC_Big_Scissor：按 F407 Y0~Y5 一比一
 * Y0 电机继电器 → RELAY0
 * Y1~Y4 电磁阀 → OUT0~OUT3（24V MOSFET）
 * Y5 子机工作阀 → RELAY1（OUT 仅 4 路，第 5 阀用继电器干接点）
 * 光耦输入统一低有效，映射层归一化为 1=触发 */
static io_in_pin_t s_in_map[IO_IN_MAX] = {
    ...
    /* [IO_IN_LOWER_LIMIT]    */ { IN5_GPIO_Port, IN5_Pin, 0, 20 },
    /* [IO_IN_REFILL_BUTTON]  */ { IN6_GPIO_Port, IN6_Pin, 0, 20 },
    /* [IO_IN_PHOTOELECTRIC]  */ { IN7_GPIO_Port, IN7_Pin, 0, 50 },
    /* [IO_IN_ROTARY_SWITCH]  */ { IN8_GPIO_Port, IN8_Pin, 0, 50 },
    /* [IO_IN_SUB_UPPER_LIMIT]*/ { IN9_GPIO_Port, IN9_Pin, 0, 20 },
};

static const char *s_in_name[IO_IN_MAX] = {
    "up", "down", "lock", "estop", "upper", "lower", "refill", "photo", "rotary", "sub_upper"
};
...
static io_out_pin_t s_out_map[IO_OUT_MAX] = {
    /* [IO_OUT_MOTOR]           */ { RELAY0_GPIO_Port, RELAY0_Pin, 1 }, /* Y0 电机继电器 */
    /* [IO_OUT_DROP_VALVE]      */ { OUT0_GPIO_Port,   OUT0_Pin,   1 }, /* Y1 下降阀 */
    /* [IO_OUT_MAIN_AIR_VALVE]  */ { OUT1_GPIO_Port,   OUT1_Pin,   1 }, /* Y2 主机气阀 */
    /* [IO_OUT_MAIN_WORK_VALVE] */ { OUT2_GPIO_Port,   OUT2_Pin,   1 }, /* Y3 主机工作阀 */
    /* [IO_OUT_SUB_AIR_VALVE]   */ { OUT3_GPIO_Port,   OUT3_Pin,   1 }, /* Y4 子机气阀 */
    /* [IO_OUT_SUB_WORK_VALVE]  */ { RELAY1_GPIO_Port, RELAY1_Pin, 1 }, /* Y5 子机工作阀 */
};
...
static uint8_t io_is_warning_input(io_in_id_t id)
{
    return (id == IO_IN_LOCK_BUTTON ||
            ...
            id == IO_IN_SUB_UPPER_LIMIT) ? 1U : 0U;
}
...
void App_IO_Map_Init(product_type_t type)
{
    ...
    io_init_input(IN5_GPIO_Port, IN5_Pin);
    io_init_input(IN6_GPIO_Port, IN6_Pin);
    io_init_input(IN7_GPIO_Port, IN7_Pin);
    io_init_input(IN8_GPIO_Port, IN8_Pin);
    io_init_input(IN9_GPIO_Port, IN9_Pin);

    /* 使用通道 */
    io_init_output(RELAY0_GPIO_Port, RELAY0_Pin, 1); /* 电机继电器 */
    io_init_output(RELAY1_GPIO_Port, RELAY1_Pin, 1); /* 子机工作阀 */
    io_init_output(OUT0_GPIO_Port, OUT0_Pin, 1);
    io_init_output(OUT1_GPIO_Port, OUT1_Pin, 1);
...
#if IO_MAP_DEBUG == 1
    elog_i("IOMAP", "[IOMAP] Large-scissor F103: Y0=RELAY0, Y1-4=OUT0-3, Y5=RELAY1");
#endif
    for (int i = 0; i < IO_IN_MAX; i++) {
...
void App_IO_EmergencyOff(void)
{
    ...
    /* 高有效输出：BSRR 高 16 位复位为关断（含 RELAY + OUT） */
    GPIOB->BSRR = ((uint32_t)(RELAY0_Pin | RELAY1_Pin | RELAY2_Pin | RELAY3_Pin)) << 16U;
    GPIOC->BSRR = ((uint32_t)(RELAY4_Pin | RELAY5_Pin |
                              OUT0_Pin | OUT1_Pin | OUT2_Pin | OUT3_Pin)) << 16U;
