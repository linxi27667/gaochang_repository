...
#define LIFT_LOG_E(...)  elog_e("LIFT", __VA_ARGS__)
#else
#define LIFT_LOG_I(...)
#define LIFT_LOG_E(...)
#endif
...
#define LIFT_TASK_PERIOD_MS     1

/* IWDG nominal timeout: 2 s at 32 kHz LSI (actual timeout follows LSI tolerance). */
#define LIFT_IWDG_KEY_START             0xCCCCU
#define LIFT_IWDG_KEY_WRITE_ENABLE      0x5555U
#define LIFT_IWDG_KEY_RELOAD            0xAAAAU
#define LIFT_IWDG_PRESCALER_64          4U
#define LIFT_IWDG_RELOAD_2S             999U
#define LIFT_IWDG_SYNC_TIMEOUT          100000U

/* LED periods are configured in milliseconds and converted to task loops. */
#define LIFT_MS_TO_LOOPS(ms)        (((ms) + LIFT_TASK_PERIOD_MS - 1U) / LIFT_TASK_PERIOD_MS)
#define LED_HEARTBEAT_NORMAL_CNT    LIFT_MS_TO_LOOPS(500U)
#define LED_HEARTBEAT_MOTION_CNT    LIFT_MS_TO_LOOPS(100U)
#define LED_HEARTBEAT_ALARM_CNT     LIFT_MS_TO_LOOPS(200U)
#define LED_HEARTBEAT_LOCKED_CNT    LIFT_MS_TO_LOOPS(1000U)
#define LED_COM_BLINK_CNT           LIFT_MS_TO_LOOPS(250U)
#define LIFT_HEARTBEAT_LOG_CNT      LIFT_MS_TO_LOOPS(5000U)

/* 诊断标志 */
volatile uint8_t g_lift_task_created = 0;
volatile uint8_t g_lift_task_started = 0;
volatile uint32_t g_lift_task_loop_cnt = 0;
static uint32_t g_lift_led_toggle_cnt = 0;

static void Lift_Watchdog_Init(void)
{
#if defined(DBGMCU_CR_DBG_IWDG_STOP)
    DBGMCU->CR |= DBGMCU_CR_DBG_IWDG_STOP;
#elif defined(DBGMCU_APB1_FZ_DBG_IWDG_STOP)
    DBGMCU->APB1FZ |= DBGMCU_APB1_FZ_DBG_IWDG_STOP;
#endif

    IWDG->KR = LIFT_IWDG_KEY_START;
    IWDG->KR = LIFT_IWDG_KEY_WRITE_ENABLE;
    IWDG->PR = LIFT_IWDG_PRESCALER_64;
    IWDG->RLR = LIFT_IWDG_RELOAD_2S;

    uint32_t timeout = LIFT_IWDG_SYNC_TIMEOUT;
    while ((IWDG->SR != 0U) && (timeout > 0U)) {
        timeout--;
    }
    IWDG->KR = LIFT_IWDG_KEY_RELOAD;
}

static inline void Lift_Watchdog_Refresh(void)
{
    IWDG->KR = LIFT_IWDG_KEY_RELOAD;
}
...
void Lift_Task_Create(void)
{
    ...
    LIFT_LOG_I("[LIFT] create stack_words=%lu free_before=%lu min_free=%lu",
    ...
    if (ret != pdPASS)
    {
        LIFT_LOG_E("[LIFT] task create failed ret=%ld free_after=%lu min_free=%lu",
               (long)ret,
    ...
    {
        LIFT_LOG_I("[LIFT] task create ok free_after=%lu min_free=%lu",
               (unsigned long)xPortGetFreeHeapSize(),
               (unsigned long)xPortGetMinimumEverFreeHeapSize());
    }
}
...
void Lift_Task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        App_RiseCounter_Poll(state_snap.state, g_current_role);
        ...
        /* 7. Heartbeat log every 5000 ms. */
        if (++heartbeat_cnt >= LIFT_HEARTBEAT_LOG_CNT)
        {
            heartbeat_cnt = 0U;
            LIFT_HEARTBEAT_LOG_I("[LIFT] hb loop=%lu state=%d dtu=%s led_tog=%lu led=%d",
