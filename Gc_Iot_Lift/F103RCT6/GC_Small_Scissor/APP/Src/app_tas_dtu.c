                                                       ...
                                                       const char *event)
{
    ...
    len = snprintf(json, sizeof(json),
                   "{\"v\":1,\"type\":\"command_response\",\"chip_uid\":\"%s\",\"uid\":\"%s\","
                    ...
                    "\"event\":\"%s\",\"maintenance\":{\"total_lift_count\":%lu,\"maintenance_lift_count\":%lu,\"maintenance_threshold\":%u,\"maintenance_count\":%lu,\"last_maintenance_total\":%lu,\"maintenance_due\":%lu,\"usage_epoch\":%lu},\"maintenance_due\":%lu,\"dtu\":{\"state\":\"%s\",\"csq\":%d}}",
                   App_TasDtu_ChipUidString(),
                   App_TasDtu_ChipUidString(),
                   "small_scissor",
    ...
    {
        tas_dtu_result_t send_result = App_TasDtu_SendJson(json);
...
static void App_TasDtu_HandleJsonCommand(const char *json)
{
        ...
        App_TasDtu_JsonHasCommand(json, "report_now"))
    {
        ...
        (void)App_TasDtu_ReportCommandResult("get_status", msg_id,
                                             (telemetry_result == TAS_DTU_RESULT_OK) ? "reported" : "telemetry_failed",
                                             (telemetry_result == TAS_DTU_RESULT_OK) ? "report_ok" : "report_failed");
        return;
    }
...
tas_dtu_result_t App_TasDtu_ConfigureMqtt(void)
{
    ...
    DTU_CONNECT_LOG_A("DTU",
           "[DTU] MQTT config start broker=%s:%u client=%s pub=%s sub=%s",
           TAS_DTU_BROKER_HOST,
           (unsigned int)TAS_DTU_BROKER_PORT,
           client_id,
           topic_up,
...
tas_dtu_result_t App_TasDtu_ReportRiseCounter(uint32_t total_count,
                                               uint32_t remainder_ms,
                                               uint32_t flash_sequence)
{
    char json[TAS_DTU_JSON_SIZE];

    if (LiftIot_BuildRiseCounterJson(json,
                                     sizeof(json),
                                     g_tas_dtu_status.tx_seq,
                                     total_count,
                                     remainder_ms,
