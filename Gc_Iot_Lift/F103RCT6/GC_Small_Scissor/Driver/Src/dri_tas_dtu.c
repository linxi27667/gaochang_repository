...
static void TasDtu_Task_InitModule(void)
{
    ...
    {
        DTU_CONNECT_LOG_E("DTU", "[DTU] USART1 BSP init failed");
        vTaskDelete(NULL);
    }

    DTU_CONNECT_LOG_A("DTU", "[DTU] USART1 ready at fixed 9600, starting MQTT %s:%u",
           TAS_DTU_BROKER_HOST,
...
static void TasDtu_Task_ServiceRiseCounter(void)
{
    ...
    /* Persist first so an accepted UART frame always has durable state behind it. */
    if (LiftIot_GetRiseCounterSaveSnapshot(&total_count,
                                           &remainder_ms,
                                           &generation) != 0U)
    {
        if (App_W25Qxx_RiseCounter_Save(total_count,
                                        remainder_ms,
                                        &flash_sequence) != W25Q_OK)
        {
            DTU_TRANSFER_LOG_W("DTU", "[DTU] rise counter flash save failed");
            return;
        }

        LiftIot_ConfirmRiseCounterSaved(generation, flash_sequence);
    }
    ...
    if (LiftIot_GetRiseCounterReportSnapshot(&total_count,
                                             &remainder_ms,
                                             &flash_sequence) == 0U)
    {
        return;
    ...
    if (App_TasDtu_ReportRiseCounter(total_count,
                                     remainder_ms,
                                     flash_sequence) == TAS_DTU_RESULT_OK)
    {
        LiftIot_MarkRiseCounterReported(flash_sequence);
    }
    else
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] rise counter report failed");
    }
}
...
static void TasDtu_Task_ReportIfDue(uint32_t now)
{
    ...
    /* Prioritize command replies on the 9600-baud half-duplex link. */
...
void TasDtu_Task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        TasDtu_Task_ServiceRiseCounter();
