...
static tas_dtu_result_t App_TasDtu_SetUartBaud(uint32_t baud)
{
    ...
    DTU_CONNECT_LOG_A("DTU", "[DTU] switch USART1 baud to %lu", (unsigned long)baud);
                                           ...
                                           uint16_t resp_size)
{
    ...
    while ((HAL_GetTick() - start) < timeout_ms)
    {
        ...
        /* 鍐呭眰寰幆澶勭悊 FIFO 鏁版嵁銆傜┖鏉夸笂 UART RX 鎮┖浼氭寔缁敹鍒板櫔澹帮紝
         * 鑻ヤ笉璁╁嚭 CPU锛孌TU 浠诲姟浼氶暱鏃堕棿鍗犵敤璋冨害锛屽共鎵?Lift_Task 蹇冭烦銆?         * 姣忓鐞?32 瀛楄妭涓诲姩 taskYIELD() 涓€娆★紝淇濊瘉 Lift_Task 鑳藉強鏃惰皟搴︺€?         */
...
static tas_dtu_result_t App_TasDtu_EnterCommandMode(void)
{
    ...
    /*
     * 涓婄數鍚庢ā鍧楅€氬父宸插湪閫忎紶鎬侊紝鍏堝彂 AT 浼氳褰撲綔涓氬姟鏁版嵁鍙戝埌 MQTT銆?     * 鍥犳鍥哄畾鍏堟寜鎵嬪唽鐢?+++ 杩涘叆鍛戒护妯″紡锛涜嫢 +++ 澶辫触锛屽啀鐢?AT 鍒ゆ柇鏄惁鏈潵灏卞湪鍛戒护妯″紡銆?     */
    (void)App_TasDtu_WaitTxIdle(2000U);
    g_tas_dtu_status.transparent_ready = 0U;
...
static uint8_t App_TasDtu_SavedMqttConfigMatches(void)
{
    ...
    DTU_CONNECT_LOG_A("DTU", "[DTU] saved MQTT config verified broker=%s:%u pub=%s sub=%s",
                      TAS_DTU_BROKER_HOST, (unsigned int)TAS_DTU_BROKER_PORT,
                      topic_up, topic_down);
    return 1U;
}
...
static tas_dtu_result_t App_TasDtu_WaitMqttConnected(uint32_t timeout_ms)
{
    ...
    do
    {
        ...
        result = App_TasDtu_WaitFor("MQTT CONNECTED", remaining,
                                    g_tas_dtu_at_resp,
                                    (uint16_t)sizeof(g_tas_dtu_at_resp));
        ...
        if (result == TAS_DTU_RESULT_AT_ERROR)
        {
            elapsed = HAL_GetTick() - start;
            remaining = (elapsed < timeout_ms) ? (timeout_ms - elapsed) : 0U;
            DTU_CONNECT_LOG_W("DTU", "[DTU] transient ERROR while waiting MQTT, continue (%lums remaining)",
                                                     ...
                                                     uint16_t resp_size)
{
    ...
    while ((HAL_GetTick() - start) < timeout_ms)
    {
        ...
        /* 鍚?App_TasDtu_WaitFor锛氶檺鍒跺唴灞傚惊鐜繛缁鐞嗗瓧鑺傛暟锛屼富鍔ㄨ鍑?CPU */
        while (BSP_UART_ReadByte(&g_tas_dtu_uart, &ch))
        {
            total_rx++;
...
static tas_dtu_result_t App_TasDtu_QueryAskConnect(void)
{
    ...
    /*
     * 涓嶈兘鍙瓑 OK銆傚疄娴嬫ā鍧楀彲鑳藉厛鍚?OK锛岄殢鍚庢墠鍚?+ASKCONNECT锛?     * 濡傛灉鎻愬墠杩斿洖浼氭妸鐪熷疄杩炴帴鐘舵€侀仐鐣欏埌涓嬩竴鏉?AT 鍝嶅簲閲屻€?     */
...
/* 鍙湪 MQTT 宸茶繛閫氬悗鐨勫惎鍔?閲嶈繛闃舵閲囨牱涓€娆★紝绂佹涓氬姟杩愯鏈熷懆鏈熸煡璇?CSQ銆?*/
                                                      ...
                                                      const char *event)
{
    ...
    len = snprintf(json, sizeof(json),
                   "{\"type\":\"status\",\"device\":\"%s\",\"seq\":%lu,\"tick\":%lu,"
                    ...
                    "\"result\":\"%s\",\"product_type\":\"%s\",\"maintenance_due\":%u,"
...
static uint8_t App_TasDtu_IsAcceptedCommandJson(const char *json)
{
    ...
    /*
     * 鍙鐞嗗钩鍙颁笅鍙戝懡浠わ紝蹇界暐鏈満 telemetry/status 鍥炵幆銆?     */
    if (strcmp(type, "command") != 0)
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] CMD rejected reason=invalid_type type=%s", type);
        return 0U;
    }

    if (uid[0] != '\0')
    {
        if (App_TasDtu_UidMatchesLocal(uid) == 0U)
        {
            DTU_TRANSFER_LOG_W("DTU", "[DTU] CMD rejected reason=target_uid_mismatch target_uid=%s", uid);
            return 0U;
        }
        return 1U;
    }
    ...
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] CMD rejected reason=device_mismatch device=%s", device);
        return 0U;
    }
...
static void App_TasDtu_HandleJsonCommand(const char *json)
{
    ...
    if (msg_id[0] == '\0')
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] CMD rejected cmd=%s reason=missing_msg_id",
                           (cmd_name[0] != '\0') ? cmd_name : "unknown");
        (void)App_TasDtu_ReportCommandResult((cmd_name[0] != '\0') ? cmd_name : "unknown",
                                             msg_id, "missing_msg_id_denied", "missing_msg_id");
        return;
    }
    ...
    {
        lift_result = LiftIot_SetLocked(1U, account);
        (void)App_TasDtu_ReportCommandResult("lock", msg_id,
                                             (lift_result == LIFT_IOT_OK) ? "locked" : "lock_denied",
                                             (lift_result == LIFT_IOT_OK) ? "lock_ok" : "lock_denied");
        return;
    }

    if (App_TasDtu_JsonHasCommand(json, "unlock"))
    {
        lift_result = LiftIot_SetLocked(0U, account);
        (void)App_TasDtu_ReportCommandResult("unlock", msg_id,
                                             (lift_result == LIFT_IOT_OK) ? "unlocked" : "unlock_denied",
                                             (lift_result == LIFT_IOT_OK) ? "unlock_ok" : "unlock_denied");
        return;
    }
    ...
    if (App_TasDtu_JsonHasCommand(json, "maintenance_done"))
    {
        ...
        (void)App_TasDtu_ReportCommandResult("maintenance_done",
                                             msg_id,
    ...
    if (App_TasDtu_JsonHasCommand(json, "reset_usage"))
    {
        ...
        (void)App_TasDtu_ReportCommandResult("reset_usage",
                                             msg_id,
                                             (lift_result == LIFT_IOT_OK) ? "usage_reset" : "usage_reset_failed",
                                             (lift_result == LIFT_IOT_OK) ? "usage_reset" : "usage_reset_failed");
        return;
    }
...
tas_dtu_result_t App_TasDtu_Init(void)
{
    ...
    BSP_UART_SetDirection(&g_tas_dtu_uart, NULL, 0); /* F103: USART1 直连 DTU，无 RS485 DE */
...
static tas_dtu_result_t App_TasDtu_FlushPendingRiseReport(void)
{
    char json[TAS_DTU_JSON_SIZE];
    lift_iot_result_t lift_result;
    tas_dtu_result_t dtu_result;

    if (LiftIot_HasPendingRiseReport() == 0U)
    {
        return TAS_DTU_RESULT_OK;
    }

    lift_result = LiftIot_BuildRiseCountJson(json, sizeof(json));
    if (lift_result != LIFT_IOT_OK)
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] pending rise report build failed");
        return TAS_DTU_RESULT_BUFFER_SMALL;
    }

    dtu_result = App_TasDtu_SendJson(json);
    if (dtu_result != TAS_DTU_RESULT_OK)
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] pending rise report send failed");
        return dtu_result;
    }

    if (LiftIot_ConfirmPendingRiseReport() == 0U)
    {
        DTU_TRANSFER_LOG_W("DTU", "[DTU] pending rise report persist confirm failed");
        return TAS_DTU_RESULT_AT_ERROR;
    }
...
tas_dtu_result_t App_TasDtu_ReportTelemetry(void)
{
    ...
    result = App_TasDtu_FlushPendingRiseReport();
    if (result != TAS_DTU_RESULT_OK)
    {
        return result;
    }
...
tas_dtu_result_t App_TasDtu_SendJson(const char *json)
{
    ...
    {
        DTU_TRANSFER_LOG_E("DTU", "[DTU] JSON rejected: bytes=%lu limit<%u",
                           (unsigned long)json_len, (unsigned int)TAS_DTU_PACKET_SIZE);
        return TAS_DTU_RESULT_BUFFER_SMALL;
    }
