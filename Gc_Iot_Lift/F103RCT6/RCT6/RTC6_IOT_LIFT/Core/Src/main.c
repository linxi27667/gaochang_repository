...
int main(void)
{
  ...
  if (App_MinSys_Init() == 0U)
  {
    elog_e("SYS", "[SYS] minimum system device init failed");
    Error_Handler();
  }

  elog_a("CLOCK",
         "[CLOCK] sys=%lu hclk=%lu pclk1=%lu pclk2=%lu adc=%lu rtos_tick=%lu",
  ...
  elog_i("SYS", "[SYS] uid=%08lX-%08lX-%08lX adc_cal=%u outputs=safe_off",
         (unsigned long)HAL_GetUIDw0(),
         (unsigned long)HAL_GetUIDw1(),
         (unsigned long)HAL_GetUIDw2(),
         (unsigned int)MinSys_Is_Adc_Ready());
  /* USER CODE END 2 */
