...
static io_in_pin_t s_in_map[IO_IN_MAX] = {
    ...
    /* [IO_IN_ROTARY_SWITCH] */ { GPIOE, GPIO_PIN_12, 1, 50 }, /* PE12 旋转开关 active-high */
...
void App_IO_Map_Init(product_type_t type)
{
    ...
    io_init_input(GPIOE, GPIO_PIN_12, 1); /* PE12 旋转开关 上拉 */
...
{
    RCC->AHB1ENR |= RCC_AHB1ENR_GPIODEN | RCC_AHB1ENR_GPIOFEN;
    (void)RCC->AHB1ENR;

    GPIOF->BSRR = ((uint32_t)(GPIO_PIN_8 | GPIO_PIN_9)) << 16U;
    GPIOD->BSRR = ((uint32_t)(GPIO_PIN_8 | GPIO_PIN_9 |
                              GPIO_PIN_10 | GPIO_PIN_11)) << 16U;
