...
#define LIFT_CORE_LOG_W(...)  elog_w(__VA_ARGS__)
#define LIFT_CORE_LOG_E(...)  elog_e(__VA_ARGS__)
#else
#define LIFT_CORE_LOG_I(...)
#define LIFT_CORE_LOG_W(...)
#define LIFT_CORE_LOG_E(...)
#endif

#if IO_MAP_DEBUG == 1
#define IO_MAP_LOG_I(...)  elog_i(__VA_ARGS__)
#define IO_MAP_LOG_W(...)  elog_w(__VA_ARGS__)
#else
#define IO_MAP_LOG_I(...)
#define IO_MAP_LOG_W(...)
#endif
...
#define DOWN_MOTOR_HOLD_MS            2500U
...
static void log_input_change(uint8_t id, uint8_t old_pressed, uint8_t new_pressed, uint8_t raw)
{
    ...
    if ((new_pressed != 0U) && (input_is_warning(id) != 0U)) {
        ...
        IO_MAP_LOG_W(tag, "[%s] triggered source=%s id=%d pin=%s state=%s %u->%u phy=%u active=%u debounce=%u",
               tag,
               s_input_name[id],
               (int)id,
               s_input_pin_name[id],
        ...
        const char *tag = input_warning_tag(id);
        IO_MAP_LOG_I(tag, "[%s] cleared source=%s id=%d pin=%s state=%s %u->%u phy=%u active=%u debounce=%u",
               tag,
               s_input_name[id],
               (int)id,
               s_input_pin_name[id],
        ...
        IO_MAP_LOG_I("IOMAP", "[IOMAP] switch changed id=%d name=%s pin=%s state=%s %u->%u phy=%u active=%u debounce=%u",
               (int)id,
               s_input_name[id],
               s_input_pin_name[id],
               input_state_text(new_pressed),
...
static uint8_t safety_guard_check(lift_ctx_t *ctx)
{
    ...
    if (in->pressed[LIFT_IN_ESTOP] != 0U) {
        if (ctx->alarm.estop_active == 0U) {
            ...
            LIFT_CORE_LOG_W("CORE", "[CORE] E-STOP triggered");
        }
        outputs_clear(&ctx->output_req);
    ...
    if (ctx->current_state == STATE_ESTOP) {
        ...
        App_OpLog_Record("EVENT_ESTOP_RECOVER", APP_OP_RESULT_OK, 0U, "buttons_released");
            ...
            LIFT_CORE_LOG_I("CORE", "[CORE] E-STOP released, back to IDLE");
    ...
    if ((in->pressed[LIFT_IN_LOWER_LIMIT] != 0U) &&
        ...
        LIFT_CORE_LOG_I("CORE", "[CORE] Photo alarm cleared by lower limit shield");
    ...
    if (photo_blocked_active(in) != 0U) {
        if (ctx->alarm.photo_alarm_latched == 0U) {
            ...
            LIFT_CORE_LOG_W("CORE", "[CORE] Photoelectric blocked, alarm triggered remote_clear=%u",
    ...
    if (ctx->current_state == STATE_PHOTO_ALARM) {
        ...
        if ((ctx->alarm.photo_alarm_latched != 0U) &&
            ...
            if (was_remote_clear != 0U) {
                LIFT_CORE_LOG_I("CORE", "[CORE] Photo alarm cleared by remote");
            } else {
                LIFT_CORE_LOG_I("CORE", "[CORE] Photo alarm auto-cleared by photo release");
            }
    ...
    if ((in->pressed[LIFT_IN_UP] != 0U) && (in->pressed[LIFT_IN_DOWN] != 0U)) {
        if (ctx->alarm.invalid_input_latched == 0U) {
            ...
            LIFT_CORE_LOG_W("CTRL", "[CTRL] Dual key conflict: motion inhibited");
        }
...
static void handle_timeout_if_needed(lift_ctx_t *ctx)
{
    ...
    App_OpLog_Record("EVENT_OPERATION_TIMEOUT", APP_OP_RESULT_FAILED, operation_elapsed(ctx), "operation_timeout");
        LIFT_CORE_LOG_E("SAFETY", "[SAFETY] operation timeout state=%s", App_LiftCore_StateName(ctx->current_state));
...
static lift_outputs_t safety_guard_before_output(lift_ctx_t *ctx, lift_outputs_t requested)
{
    ...
    if (state_input_matches(ctx) == 0U) {
        ...
        LIFT_CORE_LOG_W("SAFETY", "[SAFETY] state/input mismatch state=%s", App_LiftCore_StateName(ctx->current_state));
...
void app_lift_apply_outputs(lift_ctx_t *ctx, const lift_outputs_t *safe_outputs)
{
    ...
    if (ctx->output_actual.motor_on != safe_outputs->motor_req) {
        LIFT_CORE_LOG_I("OUT", "[OUT] PF8 motor %s", safe_outputs->motor_req ? "ON" : "OFF");
    }
    if (ctx->output_actual.down_valve_on != safe_outputs->down_valve_req) {
        LIFT_CORE_LOG_I("OUT", "[OUT] PD9 down_valve %s", safe_outputs->down_valve_req ? "ON" : "OFF");
    }
    if (ctx->output_actual.air_valve_on != safe_outputs->air_valve_req) {
        LIFT_CORE_LOG_I("OUT", "[OUT] PD8 air_valve %s", safe_outputs->air_valve_req ? "ON" : "OFF");
    }
...
void App_LiftCore_SetRemoteLock(uint8_t locked)
{
    ...
    if (next != 0U) {
        ...
        LIFT_CORE_LOG_I("CORE", "[CORE] Remote lock=%u", next);
    } else {
...
uint8_t App_LiftCore_RequestClearPhotoAlarm(const char *source)
{
    ...
    if ((s_lift_ctx.current_state == STATE_PHOTO_ALARM) &&
        ...
        (s_lift_ctx.alarm.photo_alarm_requires_remote_clear != 0U) &&
        (s_lift_ctx.input.pressed[LIFT_IN_PHOTO] == 0U) &&
        ...
        LIFT_CORE_LOG_I("CORE", "[CORE] Photo alarm clear requested by %s", source);
        return 1U;
    } else {
        s_lift_ctx.remote_clear_photo_req = 0U;
        App_OpLog_Record("clear_alarm_ignored", APP_OP_RESULT_FAILED, 0U, source);
        LIFT_CORE_LOG_I("CORE", "[CORE] Photo alarm clear ignored by %s state=%s latched=%u remote_need=%u",
               source,
               App_LiftCore_StateName(s_lift_ctx.current_state),
