...
int main(void)
{
  ...
  LiftIot_Init();
  elog_i("SYS", "[BOOT] 1 app init done");
  Dri_Lift_Init(g_product_type);
  elog_i("SYS", "[BOOT] 2 lift init returned");
  LED_POWER_ON();
  elog_i("SYS", "[BOOT] 3 power led on");
  elog_i("SYS", "[SYS] small scissor safety state machine ready product=%s",
