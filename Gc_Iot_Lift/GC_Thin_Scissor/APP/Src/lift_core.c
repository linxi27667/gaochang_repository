...
/* ============ 外部声明：各型号 ops ============ */
extern const lift_ops_t thin_scissor_ops;   /* lift_thin_scissor.c */
/* 其他型号预留：
...
void LiftCore_Init(void)
{
    ...
    switch (g_product_type) {
        case PRODUCT_TYPE_THIN_SCISSOR:
            g_lift_ops = &thin_scissor_ops;
            break;
        /* 其他型号预留 */
        default:
            #if LIFT_CORE_DEBUG == 1
            elog_e("CORE", "[CORE] No ops for product_type=%d, fallback to thin_scissor",
                   g_product_type);
            #endif
            g_lift_ops = &thin_scissor_ops;
            break;
...
void LiftCore_Poll(void)
{
    ...
    /* 2. 光电报警检测（第二优先级）
     * 超薄小剪下限位触发时屏蔽光电（车辆已落地，遮挡是正常的）。 */
    uint8_t skip_photo = App_IO_Read(IO_IN_LOWER_LIMIT);
