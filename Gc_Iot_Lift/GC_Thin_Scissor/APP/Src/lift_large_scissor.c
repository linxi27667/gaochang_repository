...
typedef enum {
    ...
    LS_DOWN_PHASE_2,      /* 下降阶段2：气阀已开，等待电机保持时间 (2500/1500ms) */
...
#if LARGE_SCISSOR_DEBUG == 1
            elog_i("LSCS", "[LSCS] UP air valve ON, role=%s", App_Product_RoleName(role));
#endif
        }
    }
...
#if LARGE_SCISSOR_DEBUG == 1
    elog_i("LSCS", "[LSCS] DOWN start role=%s", App_Product_RoleName(role));
#endif
}
...
static void ls_poll_down(lift_role_t role)
{
...
#if LARGE_SCISSOR_DEBUG == 1
            elog_i("LSCS", "[LSCS] DOWN air valve ON, role=%s", App_Product_RoleName(role));
#endif
        }
    }
    /* 阶段2→3：电机保持时间到，电机断开，下降阀打开 */
    else if (s_ls_state == LS_DOWN_PHASE_2) {
        if ((now - s_action_start_tick) >= hold_ms) {
            App_IO_Write(IO_OUT_MOTOR, 0);
            App_IO_Write(IO_OUT_DROP_VALVE, 1);
            s_ls_state = LS_DOWN_DROPPING;
#if LARGE_SCISSOR_DEBUG == 1
            elog_i("LSCS", "[LSCS] DOWN motor OFF, drop valve ON, role=%s", App_Product_RoleName(role));
#endif
        }
    }
    ...
    if (role == LIFT_ROLE_MAIN) {
        ...
        if (lower_limit) {
            ...
            ls_stop_all_to_idle();
            uint8_t detail = (uint8_t)role;
            App_OpLog_Record(OP_DOWN_STOP_LIMIT, OP_RESULT_OK, dur, &detail, 1);
#if LARGE_SCISSOR_DEBUG == 1
            elog_i("LSCS", "[LSCS] DOWN stop by lower limit, role=%s dur=%u",
                   App_Product_RoleName(role), dur);
#endif
            return;
        }
    }
...
#if LARGE_SCISSOR_DEBUG == 1
        elog_i("LSCS", "[LSCS] DOWN force drop: limit=%u lock=%u", upper_limit, lock_pressed);
#endif
        /* 不退出，保持下降状态直到按钮释放 */
    }
