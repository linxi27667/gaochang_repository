...
function getPlatformSnapshot(user = null) {
  ...
  // 非 admin 通过 device_bindings 多对多鉴权
  const deviceScope = isAdmin ? '' : `WHERE EXISTS (
  ...
  // alarms / maintenance / commands / logs 统一使用 EXISTS 子查询鉴权
  const itemScope = isAdmin ? '' : `WHERE EXISTS (
    SELECT 1 FROM device_bindings b
    WHERE b.device_id = d.device_id AND b.user_id = ? AND b.status = 'active'
  )`;
  ...
  // 日志:非 admin 只看自己产生的日志,或绑定设备的日志
  const logScope = isAdmin ? '' : `WHERE l.user_id = ? OR EXISTS (
    SELECT 1 FROM device_bindings b
    WHERE b.device_id = d.device_id AND b.user_id = ? AND b.status = 'active'
  )`;
