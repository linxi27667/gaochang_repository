...
const BROKER = 'mqtt://8.134.167.240:1883';
const API_BASE = 'http://localhost:3000/api';
const TOPIC_PREFIX = 'gaochang/lift';
const GATEWAY_ID = 'f407zet6';
const DEVICE_ID = 'gaochang_lift_f407zet6';
const DEVICE_NAME = 'Gaochang-Lift-F407-01';
const TOPIC_TELEMETRY = `${TOPIC_PREFIX}/${GATEWAY_ID}/telemetry`;
const TOPIC_COMMAND = `${TOPIC_PREFIX}/${GATEWAY_ID}/command`;
...
async function runTests() {
  ...
  // ===== Test 2: 设备管理API =====
  log('TEST', '\n--- Test 2: 设备管理API ---');
  const devices = await apiRequest('GET', '/devices', null, token);
  ...
  mqttClient.publish(TOPIC_TELEMETRY, JSON.stringify({
    type: 'telemetry', device: DEVICE_ID, device_id: DEVICE_ID, name: DEVICE_NAME,
    online: true, locked: false, state: 'up', alarm: 'none',
  ...
  mqttClient.publish(TOPIC_TELEMETRY, JSON.stringify({
    type: 'telemetry', device: DEVICE_ID, device_id: DEVICE_ID, name: DEVICE_NAME,
    online: true, locked: false, state: 'stop', alarm: 'stall',
